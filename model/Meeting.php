...
class Meeting extends BaseModel implements JsonSerializable
{
    ...
    public static function create(string $date, File $file, Member $member): Meeting
    {
        ...
        $id = $DB->query(
            "INSERT INTO REUNION (date_reunion, fichier_reunion, id_membre)
             VALUES (?, ?, ?)",
            "ssi",
            [$date, $file->getFileName(), $member->getId()]
        );
    ...
    {
        $data = $this->DB->select(
            "SELECT id_reunion, date_reunion, fichier_reunion
             FROM REUNION
             WHERE id_reunion = ?",
            "i",
            [$this->id]
