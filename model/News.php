...
class News extends BaseModel implements JsonSerializable
{
    public static function create(string $nom, string $description, string $date, int $id_membre, File|null $image): News
    {
        ...
        $id = $DB->query(
            "INSERT INTO ACTUALITE (titre_actualite, contenu_actualite, date_actualite, id_membre, image_actualite)
             VALUES (?, ?, ?, ?, ?)",
            "sssis",
            [$nom, $description, $date, $id_membre, $imageFileName]
        );
    ...
    public function update(string $nom, string $description, string $date, int $id_membre): News
    {
        $this->DB->query(
            "UPDATE ACTUALITE
             SET titre_actualite = ?, contenu_actualite = ?, date_actualite = ?, id_membre = ?
             WHERE id_actualite = ?",
            "sssii",
            [$nom, $description, $date, $id_membre, $this->id]
        );
    ...
    public static function getInstance(int $id): ?News
    {
        $DB = new DB();
        $result = $DB->select(
    ...
    {
        $data = $this->DB->select(
            "SELECT A.*, M.prenom_membre, M.prenom_membre
             FROM ACTUALITE AS A
