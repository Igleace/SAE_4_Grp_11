...
class Accounting extends BaseModel implements JsonSerializable
{
    public static function create(string $date, string $name, string $url, int $memberId): Accounting
    {
        ...
        $id = $DB->query(
            "INSERT INTO COMPTABILITE (date_comptabilite, nom_comptabilite, url_comptabilite, id_membre)
             VALUES (?, ?, ?, ?)",
            "sssi",
            [$date, $name, $url, $memberId]
        );
    ...
    {
        $this->DB->query(
            "DELETE FROM COMPTABILITE WHERE id_comptabilite = ?",
            "i",
            [$this->id]
        );
    }
