...
class Grade extends BaseModel implements JsonSerializable
{
    public static function create(string $name, string $description, float $price, File|null $image, float $reduction): Grade
    {
        ...
        $imageFileName = $image?->getFileName() ?? 'default_grade.png';

        $id = $DB->query(
            "INSERT INTO GRADE (nom_grade, description_grade, prix_grade, image_grade, reduction_grade)
         VALUES (?, ?, ?, ?, ?)",
            "ssdsd",
            [$name, $description, $price, $imageFileName, $reduction]
        );
    ...
    public function update(string $name, string $description, float $price, float $reduction): Grade
    {
        $this->DB->query(
            "UPDATE GRADE
             SET nom_grade = ?, description_grade = ?, prix_grade = ?, reduction_grade = ?
             WHERE id_grade = ?",
            "ssdsi",
            [$name, $description, $price, $reduction, $this->id]
        );
    ...
    public function getImage(): File|null
    {
        $image = $this->DB->select(
            "SELECT image_grade FROM GRADE WHERE id_grade = ?",
            "i",
            [$this->id]
        )[0]['image_grade'];
    ...
    public function delete(): void
    {
        ...
        $this->DB->query(
            "UPDATE GRADE SET deleted = true WHERE id_grade = ?",
            "i",
            [$this->id]
        );
    }
