...
class Cart
{
    ...
    public function getItems()
    {
        ...
        $placeholders = implode(",", array_fill(0, count($ids), "?"));
        $query = "SELECT id_article, nom_article, prix_article, stock_article, image_article FROM ARTICLE WHERE id_article IN ($placeholders)";
        $types = str_repeat("i", count($ids));

        $products = $this->db->select($query, $types, $ids);
        ...
        foreach ($products as $product) {
            ...
            $items[] = [
                ...
                'price' => $product['prix_article'],
                'quantity' => $_SESSION['cart'][$id],
                'stock' => $product['stock_article'],
                'image' => $product['image_article']
            ];
        }
        return $items;
    }
