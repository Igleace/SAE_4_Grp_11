...
class Member extends BaseModel implements JsonSerializable
{
    ...
    public static function create(string $nom, string $prenom, string $email, File|null $pp, string $tp): Member
    {
        ...
        $passwordHash = password_hash('changeme', PASSWORD_DEFAULT);
        $ppFileName = $pp?->getFileName() ?? 'default_user.png';

        $id = $DB->query(
            "INSERT INTO MEMBRE (nom_membre, prenom_membre, email_membre, pp_membre, tp_membre, password_membre) 
         VALUES (?,?,?,?,?,?)",
            "ssssss",
            [$nom, $prenom, $email, $ppFileName, $tp, $passwordHash]
        );
